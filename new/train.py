...
def compile_stage_A(model, lr=1e-4):
    ...
    for lay in chan.attacks:
        # lay.trainable = False
        if hasattr(lay, 'prob'):
            lay.prob = 20. #set low strength

    model.compile(
        optimizer=tf.keras.optimizers.Adam(lr),
        loss= {'bits_pred': bit_consistency_loss,
               'wm_pcm': None},
        metrics={'bits_pred': [BitErrorRate(name='ber')],
                 'wm_pcm': None},
    )


def compile_stage_B(model,
    ...
    model.compile(
        ...
        loss      = {'bits_pred': bits_loss,
                     'wm_pcm'   : pcm_loss},
        metrics   = {'bits_pred': [BitErrorRate(name='ber')],
                     'wm_pcm'   : None}  #  audio head purely for loss
    )
...
def callbacks_stage_A(ckpt_path, patience=5, log_prefix="stageA"):
    ...
    log_dir  = os.path.join("logs",
                            f"{log_prefix}_{run_name}" if log_prefix else run_name)
...
if __name__ == "__main__":
    ...
    # ---------------- stage logic -----------------------------------------
    if args.stage == "A":
        model_a = build_model(K=args.frames, P=args.bits,
        ...
        base = tf.keras.models.load_model(args.ckpt,
                                          custom_objects=custom,
                                          compile=False)
        model_b = build_model(K=args.frames, P=args.bits,
        ...
        # ----- stage B -----
        base = tf.keras.models.load_model(f"checkpoints/sA_{args.name}.h5",
                                          custom_objects=custom,
                                          compile=False)
        model_b = build_model(K=args.frames, P=args.bits,
                              trainable_alpha=True)
