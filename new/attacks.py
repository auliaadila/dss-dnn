...
class LowpassFIR(tf.keras.layers.Layer):
    ...
    def get_config(self):
        cfg = super().get_config(); cfg.update(dict(cutoff=self.cutoff, prob=self.prob))
        return cfg
...
class AdditiveNoise(tf.keras.layers.Layer):
    ...
    def call(self,x):
        ...
        return tf.cond(cond,_f,lambda: x)
    def get_config(self):
        c=super().get_config(); c.update(dict(strength=self.strength, prob=self.prob)); return c
...
class CuttingSamples(tf.keras.layers.Layer):
    ...
    def call(self,x):
        ...
        def _f():
            ...
            mask=tf.tensor_scatter_nd_update(tf.ones_like(flat),tf.expand_dims(pos,1),tf.zeros_like(pos,tf.float32))
            return tf.reshape(flat*mask,(B,T,1))
        return tf.cond(cond,_f,lambda: x)
    def get_config(self):
        c=super().get_config(); c.update(dict(num=self.num, prob=self.prob)); return c
...
class AttackPipeline(tf.keras.layers.Layer):
    ...
    def get_config(self):
        cfg = super().get_config()
        cfg.update(dict(attacks=[
            tf.keras.utils.serialize_keras_object(a) for a in self.attacks
        ]))
        return cfg
