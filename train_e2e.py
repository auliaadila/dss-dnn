...
def main():
    ...
    ap.add_argument('--train_dir',required=True, default="dataset/dummy/train")
    ap.add_argument('--val_dir',required=True, default="dataset/dummy/val")
    ...
    model.compile(
        optimizer=opt,
        loss={  # utk gradient
            "res_w": residual_distribution_loss,
            "pcm_w": [l1_loss, spectral_loss, snr_loss],
            "bits_pred": bit_consistency_loss,
        },
        loss_weights={
            "res_w": 0.01,  # Lower weight for residual regularization
    ...
    model.save_weights("{}_initial.h5".format(args.output))
    ...
    if args.logdir is not None:
        logdir = "{}/{}_logs".format(args.logdir, args.output)
