...
# from components.extractorlp import LPDSExtractor as LPDSSExtractor
...
def build_model_gpu(frame=160, K=15, P=64,
    ...
    # extractor = LPDSSExtractor(payload_bits=P, context_len=K * frame)
...
def main():
    ...
    if args.stage in ["A", "AB"]:
        ...
        if embedding_layer: #xhange this

            embedding_layer.alpha_settings = args.alphaset #{adaptive, constant}
            embedding_layer.resid_sign = args.residsign
            # embedding_layer.trainable_alpha = args.trainable_alpha # False by defaul
    ...
    if args.stage in ["B", "AB"]:
        ...
        if embedding_layer:
            embedding_layer.alpha_settings = args.alphaset #{adaptive, constant}
            embedding_layer.resid_sign = args.residsign
            embedding_layer.trainable_alpha = True
     
            print("alpha_net trainable?", embedding_layer.alpha_net.trainable)
            print("alpha_const trainable?", embedding_layer.alpha_const.trainable)
